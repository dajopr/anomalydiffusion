        ...
        **kwargs,
        ...
        for sample_name, anomaly_name in sample_anomaly_pairs:
            ...
            mask_path = os.path.join(
                self.data_root, sample_name, "ground_truth", anomaly_name
            )
            ...
            mask_files = [
                os.path.join(mask_path, file_name) for file_name in mask_files
            ]
        ...
        if per_image_tokens:
            assert (
                self.num_images < len(per_img_token_list)
    ...
    def __getitem__(self, idx):
        ...
        if self.per_image_tokens and np.random.uniform() < self.mixing_prob:
            text = random.choice(imagenet_dual_templates_small).format(
                placeholder_string, per_img_token_list[idx % self.num_images]
            )
        else:
...
class Personalized_mvtec_mask(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        l = len(mask_files) // 3
        self.img_files = [
            os.path.join(self.img_path, file_name) for file_name in img_files[:l]
        ]
        self.mask_files = [
            os.path.join(self.mask_path, file_name) for file_name in mask_files[:l]
        ]
...
class Positive_sample_with_generated_mask(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # mask_files.sort(key=lambda x: int(x[:3]))
        self.img_files = [
            os.path.join(self.img_path, file_name) for file_name in img_files
        ]
        self.mask_files = [
            os.path.join(self.mask_path, file_name) for file_name in mask_files
        ]
