...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        self.logvar = torch.full(
            fill_value=logvar_init, size=(self.num_timesteps,), device="cuda"
        )
...
class LatentDiffusion(DDPM):
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox"]:
                    xc = batch[
                        cond_key
                    ]  # 执行这条：xc=['a rendition of the *', 'a close-up photo of the *', 'the photo of a *']
            ...
            if not self.cond_stage_trainable or force_c_encode:  # 不执行
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c, position = self.get_learned_conditioning(
                        xc, x=masked_img, name=name
                    )
                    total_dict["position"] = position
                else:
                    c, position = self.get_learned_conditioning(
                        xc.to(self.device), x=masked_img, name=name
                    )
            else:  # 执行这条
                c = xc
    ...
    def apply_model(
        self, x_noisy, t, cond, return_ids=False, mask=None, weight_map=None, **kwargs
    ):
        ...
        if hasattr(self, "split_input_params"):  # 不执行
            ...
            x_recon = self.model(
                x_noisy, t, **cond, mask=mask, weight_map=weight_map, **kwargs
            )
        ...
        if sample:
            ...
            if (
                ...
                with self.ema_scope("Plotting Quantized Denoised"):
                    ...
                    # samples, z_denoise_row = self.sample(cond=c, batch_size=N, return_intermediates=True,
                    #                                      quantize_denoised=True)
                x_samples = self.decode_first_stage(
                    samples.to(self.device), total_dict["mask"]
                )
        ...
        if inpaint:
            ...
            with self.ema_scope("Plotting Inpaint"):
                samples, z_denoise_row = self.sample_log(
                    ...
                    x0=z[:N],
                    mask=mask,
                    embedding_position=total_dict["position"],
                    adaptive_mask=adaptive_mask,
                    embedding_manager=self.cond_stage_model,
                    decoder=self.decode_first_stage,
                )
                # if adaptive_mask:
    ...
    def optimize_sample(self, z, source_img, mask):
        ...
        for i in range(100):
            ...
            print(
                "total_loss=%.4f,recon_loss:%.4f,anomaly_loss:%.4f"
                % (loss, recon_loss, anomalay_loss)
            )
            loss.backward()
            optimizer_z.step()
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if self.test_dataset:
            ...
            save_dir = "save_models/%s/%s/%s" % (
                sub_dir,
                self.sample_name,
                self.anomaly_name,
            )
            os.makedirs(
                "save_models/%s/%s" % (sub_dir, self.sample_name), exist_ok=True
            )
            os.makedirs(
                "save_models/%s/%s/%s" % (sub_dir, self.sample_name, self.anomaly_name),
                exist_ok=True,
            )
            self.embedding_manager.save(os.path.join(save_dir, "embeddings.pt"))
            self.embedding_manager.save(
                os.path.join(save_dir, f"embeddings_gs-{self.global_step}.pt")
            )
            return
        if os.path.isdir(self.trainer.checkpoint_callback.dirpath):
            if self.spatial_encoder:
                torch.save(
                    self.embedding_manager.spatial_encoder_model.state_dict(),
                    os.path.join(
